...
try:
    ...
    query = "SELECT email, Kode_OTP FROM user WHERE id_user = %s"
    user_id = 1  # Assuming the user's ID is 1
    cursor.execute(query, (user_id,))
    row = cursor.fetchone()  # Assuming the query returns exactly one result

    recipient_email = row[0]  # Extracting email from the result
    otp = row[1]  # Extracting OTP from the result
    ...
    html_body = html_body.replace("<div class=\"otp\"></div>", f"<div class=\"otp\">{otp}</div>")
